    ...
    for mask, color in sorted_annot_and_color:
        ...
        if borders:
            contours, _ = cv2.findContours(
                np.array(
                    mask, dtype=np.uint8), cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE
            )
            cv2.drawContours(
...
class SAVDataset:
    ...
    def read_frames(self, mp4_path: str) -> Optional[List[np.ndarray]]:
        """
        ...
        if not os.path.exists(mp4_path):
            ...
            print(
                f"There are {len(frames)} frames decoded from {mp4_path} (24fps)."
            )
    ...
    def get_frames_and_annotations(
        self, video_id: str
    ) -> Tuple[Optional[List[np.ndarray]], Optional[Dict], Optional[Dict]]:
        """
        ...
        manual_annot_path = os.path.join(
            self.sav_dir, video_id + "_manual.json")
    ...
    def visualize_annotation(
        ...
        if show_auto and auto_annot is not None:
            ...
            colors.extend(
                self.auto_mask_colors[: len(
                    auto_annot["masklet"][annotated_frame_id])]
            )
